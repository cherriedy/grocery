...
public class ImportBillAdapter extends BaseAdapter {

    private Context context;
    private List<DetailedGoodsReceivedNote> list;

    public ImportBillAdapter(Context context, List<DetailedGoodsReceivedNote> list) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ViewHolder holder;
        if(convertView == null){
            convertView = LayoutInflater.from(context).inflate(R.layout.import_bill_item, parent, false);
        ...
        DetailedGoodsReceivedNote detail = list.get(position);
        Product product = detail.getProduct();
        holder.tvName.setText(product.getProductName());
        holder.tvQuantity.setText(String.valueOf(detail.getQuantity())); // Ensure to convert int to String
        holder.tvPrice.setText(String.valueOf(product.getInPrice()));
